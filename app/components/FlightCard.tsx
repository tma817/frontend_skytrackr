            ...
            <div className="flex flex-row gap-20 p-5 md:flex-row md:items-center md:gap-6">
                <div className="flex-1 flex flex-col gap-4">
                    ...
                    <div className="flex flex-row w-full justify-between items-center">
                       <div>
                            <div className="text-xl font-extrabold tracking-tight text-slate-900">{depTime}</div>
                            <div className="mt-0.5 text-xs font-semibold text-slate-500">{depCode}</div>
                        </div>
                        <div className="relative mx-8 flex flex-1 flex-col items-center min-w-[120px]">
                            ...
                            <div className={`mt-1 text-[10px] font-black uppercase tracking-widest ${isDirect ? 'text-green-500' : 'text-slate-400'}`}>
                                {stopsText}
                            </div>
                        </div>
                        <div>
                            <div className="text-xl font-extrabold tracking-tight text-slate-900">{arrTime}</div>
                            <div className="mt-0.5 text-xs font-semibold text-slate-500">{arrCode}</div>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex flex-col gap-2 rounded-xl px-4 py-2 justify-center items-center">
                    ...
                    <button 
                        className="mt-4 w-full group/btn relative flex items-center justify-center gap-2 overflow-hidden rounded-xl bg-black px-6 py-3 text-sm font-bold text-white transition-all hover:bg-black active:scale-95 shadow-lg shadow-slate-200"
                    >
                        <span>Select</span>
                        <svg className="h-4 w-4 transition-transform group-hover/btn:translate-x-1" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
