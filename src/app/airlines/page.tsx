...
export default function AirlinesPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F7F8FA]">
      ...
      <div className="mx-auto max-w-5xl px-6 py-8 flex gap-6 items-start">
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden divide-y divide-slate-100">
              {filtered.map(airline => (
                <button
                  ...
                  className={`w-full flex items-center gap-4 px-5 py-4 text-left transition-colors ${selected?.iata === airline.iata ? "bg-slate-50" : "hover:bg-slate-50"}`}
                >
                  ...
                  <div className="w-40 shrink-0 text-left">
                    <p className="text-sm font-black text-slate-900">{airline.name}</p>
                  ...
                  <div className="flex-1 hidden sm:block text-left">
                    ...
                    <p className="text-sm text-slate-700 font-medium truncate">{airline.hub}</p>
                  </div>
                  ...
                  <span className={`shrink-0 text-[10px] font-black px-2.5 py-1 rounded-full ${ALLIANCE_COLORS[airline.alliance] ?? "bg-slate-100 text-slate-500"}`}>
                    {airline.alliance}
                  </span>
        ...
        {/* Detail panel */}
        {selected && (
          <div className="w-80 shrink-0 sticky top-8 bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            {/* Header */}
            <div className="flex items-center gap-3 px-5 py-4 border-b border-slate-100">
              ...
              <button onClick={() => setSelected(null)} className="text-slate-300 hover:text-slate-600 text-lg leading-none ml-1 shrink-0">✕</button>
            </div>
